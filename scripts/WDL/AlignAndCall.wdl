...
workflow AlignAndCall {
  ...
  input {
    ...
    Float? verifyBamID
    String gatk_path
    String gatk_local_path
    String picard_path
    String haplocheck_path
  ...
  call AlignAndMarkDuplicates.AlignmentPipeline as AlignToMt {
    input:
      ...
      gatk_path = gatk_path,
      gatk_local_path = gatk_local_path,
...
task GetContamination {
  ...
  java -jar ~{haplocheck_path} "${PARENT_DIR}"
...
task M2 {
  ...
  command <<<
      ...
      export GATK_LOCAL_JAR=~{gatk_local_path}

      # We need to create these files regardless, even if they stay empty
      touch bamout.bam

      ~{gatk_path} --java-options "-Xmx~{command_mem}m" Mutect2 \
        -R ~{ref_fasta} \
        -I ~{input_bam} \
...
task MergeStats {
  ...
  command{
    ...
    ~{gatk_path} MergeMutectStats --stats ~{shifted_stats} --stats ~{non_shifted_stats} -O raw.combined.stats
  }
  output {
